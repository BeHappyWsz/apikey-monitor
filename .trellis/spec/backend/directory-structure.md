...
| Pure parse / probe / export (no DB) | `core.py` |
...
??? core.py                # parse_import_text, classify, health_check, export_*
...
- **`core.py` must stay free of DB access.** Probe and export helpers take plain dicts/strings.
...
- Putting probe HTTP calls inside `db.py` or SQL inside `core.py`.
...
## `core.py` contents (keep pure)

Own these here ? not in router or db:

- `normalize_base_url`, `join_api_path`
- `parse_import_text` / `parse_paste` (env, curl, JSON backup shapes)
- `classify`, `health_check`, `model_check` and protocol probes
- `export_config`, `export_batch`

No `sqlite3` imports in `core.py`.
