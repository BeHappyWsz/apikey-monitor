...
## Project appendix: apikey-monitor

Map generic ?layers? onto this repo:
...
| Boundary | Contract to keep aligned |
|----------|---------------------------|
| List API ? cards | Masked fields only; `models` as array |
| Edit form ? PUT | Empty `api_key` means keep; URL/key change resets status |
| Settings UI ? POST settings ? listen process | Saving host/port does not move the socket until restart |
| Batch 202 ? task poll | In-memory tasks die on process restart |
| Import JSON ? `parse_import_text` ? export JSON | Round-trip fields: name, base_url, api_key, check_model |
| Monitor ? KeyService | Uses `health=True`, not full classify every time |

Before changing a field, search both `static/` and Python packages, then update `docs/api.md` and tests.
