# PRD: 多机数据共享与坚果云 WebDAV 同步（**PARKED / 待办**）

## Status

**Deferred / backlog.** 仅记录需求与分期计划，**当前不开始** design / implement。  
恢复时：先确认分期范围 → 补 `design.md` / `implement.md` → `task.py start`。
...
### Phase 0 — 文档与约定（可先做，无代码或极少代码）

- [ ] README / design：多机使用方案对比（JSON 备份、外置 DB、WebDAV、内网单服务）  
- [ ] 明确 **单写者**：同一 `data.db` / 同一 WebDAV 文件，同时只允许一台在写  
- [ ] 文档说明环境变量 `APIKEYCONFIG_DB_PATH` 用法与风险（网盘同步 DB）  
- [ ] 安全警告：备份/云端文件 = 全量密钥  

### Phase 1 — MVP WebDAV（推荐首期实现）

- [ ] 设置页：WebDAV `server` / `username` / `password` / `remote_path`  
- [ ] 密码本地持久化；API 仅返回「是否已设置」，不回传明文  
- [ ] **测试连接**（HEAD / PROPFIND / 轻量 GET）  
- [ ] **上传**：当前库 → 与现有备份同结构的 JSON → WebDAV `PUT`  
- [ ] **下载**：WebDAV `GET` → 用户选 **合并（跳过重复）** 或 **全量替换** → 走现有 import  
- [ ] 展示上次同步结果、远程 `Last-Modified`（若可得）  
- [ ] **零第三方依赖**：`urllib` + Basic Auth + HTTPS  
- [ ] 单元测试：URL 拼接、路径规范化、导入合并去重逻辑（可 mock HTTP）  
- [ ] README：坚果云示例（`https://dav.jianguoyun.com/dav/`、应用密码、路径示例）  
...
## Non-goals for this parked task
...
1. 与用户确认做 Phase 0 only / Phase 0+1 / 含加密  
2. 写 `design.md`（模块落点：`core` 导出、`services`、设置 API、设置 UI）  
3. 写 `implement.md` 与验收用例  
4. `task.py start` 后按 implement 推进  
