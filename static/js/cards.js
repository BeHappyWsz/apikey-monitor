...
export function renderCard(key, state) {
  ...
  const modelsCount = Number(key.models_count ?? models.length) || 0;
  const modelState = key.model_status || "unknown";
  const strictModelVerified = Number(key.model_verification_version || 0) >= 1;
  ...
  const sortable = canReorder(state.status, state.query, state.sort, state.filters);
  const tags = key.tag_list || [];
  return `<article class="key-card status-${tone}${accessProblem ? " access-problem" : ""}" data-id="${key.id}" ${sortable ? 'draggable="true"' : ""}>
    ...
    <details class="card-details"><summary>模型、备注与错误详情</summary><div><p><b>模型：</b>${models.length ? models.slice(0, 8).map((model) => `<span class="chip">${esc(model)}</span>`).join(" ") : (modelsCount ? `共 ${modelsCount} 个` : "暂无")} ${(models.length || modelsCount) ? `<button class="link-btn js-models">查看列表 ${models.length || modelsCount}</button>` : ""}</p>${tags.length ? `<p><b>标签：</b>${tags.map((tag) => `<span class="chip tag-chip">${esc(tag)}</span>`).join(" ")}</p>` : ""}${key.check_model ? `<p><b>最近严格验证：</b>${strictModelVerified ? relativeTime(key.model_last_check_at) : "未完成"}</p>` : ""}<p class="count-summary"><b>检测统计：</b>监测 ${monitorCount} 次 · 严格验证 ${strictCount} 次 <button class="link-btn js-history" type="button">查看历史</button></p>${(key.model_probe_adapter && strictModelVerified && key.model_status === "up") ? `<p><b>验证壳：</b>${esc(adapterLabel[key.model_probe_adapter] || key.model_probe_adapter)}</p>` : ""}${key.notes ? `<p><b>备注：</b>${esc(key.notes)}</p>` : (key.has_notes ? `<p><b>备注：</b>已设置</p>` : "")}${(key.last_error && key.status !== "up") ? `<p class="error-line"><b>错误：</b>${esc(key.last_error)}</p>` : ""}${(key.model_last_error && strictModelVerified && key.model_status !== "up") ? `<p class="error-line"><b>模型错误：</b>${esc(key.model_last_error)}</p>` : ""}</div></details>
