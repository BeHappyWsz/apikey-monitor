...
export function renderCard(key, state) {
  ...
  return `<article class="key-card status-${tone}" data-id="${key.id}" ${sortable ? 'draggable="true"' : ""}>
    ...
    <div class="card-body-grid">
      ...
      <div class="metric access-metric" title="${esc(advice.title)}"><span>接入</span><b class="access-state ${advice.tone}" title="${esc(advice.title)}"><em>${esc(advice.label)}</em><small>${esc(advice.tag)}</small></b></div>
    </div>
