...
感谢你愿意改进 **apiKeyConfig**。本项目目标是保持小而专注：本地运行、零第三方依赖、开箱即用。
...
1. **尽量不引入第三方依赖**（Python 标准库 + 原生前端）。若确需依赖，请先在 Issue 讨论。
2. **密钥安全**：列表/详情接口不要返回明文 `api_key`；完整密钥仅限 secret / 导出 / 检测路径。默认本机单用户，**不要**引入 Web 登录/访问密码 unless 产品方向变更并先开 Issue。
