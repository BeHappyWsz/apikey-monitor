...
| `webdav_server` / `webdav_username` / `webdav_remote_path` | WebDAV 地址、用户名和远程备份路径 |
| `_webdav_password` / `_webdav_last_sync` | WebDAV 应用密码和最近同步状态（私有项） |
...
- WebDAV 应用密码以 `_webdav_password` 存于 settings 表；`SettingsService.get()` 对 API **掩码**（只回 `has_password`），且运行时不会把该密码写入 `config.json`。
