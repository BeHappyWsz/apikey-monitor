...
| `monitor_count` | 连通性/健康监测累计次数；`update_status` 落库时 +1 |
| `strict_count` | 严格模型验证累计次数；`update_model_status` 落库时 +1 |
| `sort_order` | 拖拽排序 |
...
- **严格模型验证适配**：OpenAI-compatible 模型验证先尝试 `/chat/completions`，当该路径不可用（404 / 连接失败）时再尝试 `/responses`；token 预算为 `MODEL_PROBE_MAX_TOKENS`（当前 32，含 `max_tokens` / `max_output_tokens`）。响应校验走 `model_response_evidence`：除 content / reasoning 外，拒绝「截断且无可用输出」的 200（`finish_reason`/`stop_reason` 为 `length`/`max_tokens` 且无可见文本或推理内容，错误前缀 `truncated completion`）；仍接受 chat content parts、仅有 `reasoning`/`reasoning_content`/`thinking` 或 `<think>` 块的推理型返回，以及 Anthropic `thinking` 类 content block；responses 接受 `output_text` 与嵌套 `output[].content[].text`，`status=incomplete` 且无输出时同样记为截断。401/403 与 429 不 fallback，避免把真实鉴权或限流结果掩盖成其它端点状态。严格验证成功后持久化 `model_probe_adapter`，前端只依据该字段给出接入建议：`openai_chat` 可直接用于 ccswitch 的 OpenAI chat 形态，`openai_responses` 需要 Responses 兼容壳，`anthropic_messages` 需要 Messages 协议适配。
- **定时监测（`health_check`）**：
...
**已做（列表效率）**：
1. 列表投影字段：`GET /api/keys/page` 瘦 payload（`view=list`，无 `models[]`/`notes` 正文，提供 `models_count`/`has_notes`）；详情/编辑/模型列表按需 `GET /api/keys/{id}`
2. 前端局部 DOM 更新：按 id + fingerprint 复用未变更卡片节点，避免整表 `innerHTML` 重绘

**已做（事件通知）**：
3. `GET /api/keys/events` 提供经认证的 SSE revision 事件和心跳；写路径在进程内广播 revision，前端立即显示既有的「发现更新」提示但不会自动替换滚动中的分页列表。SSE 正常时固定 revision 轮询退化为最长每 60 秒一次的恢复性检查。
