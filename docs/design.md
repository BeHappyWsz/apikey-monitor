...
- **严格模型验证适配**：OpenAI-compatible 模型验证先尝试 `/chat/completions`，当该路径不可用（404 / 连接失败）时再尝试 `/responses`；响应校验接受 chat `choices[].message.content`、responses `output_text` 与嵌套 `output[].content[].text`。401/403 与 429 不 fallback，避免把真实鉴权或限流结果掩盖成其它端点状态。
- **定时监测（`health_check`）**：
